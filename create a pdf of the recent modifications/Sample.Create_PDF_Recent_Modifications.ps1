...
#------------------------create a pdf of the txt file-----------------------------------
# File paths
$txtFilePath = "$workingDirectory\updated.txt"
...
foreach ($line in Get-Content -Path $txtFilePath) {
    $lines += $line
}
...
while ($lines.Length - ($i * $linesPerGroup) -gt 0) {
    ...
    $txtContent = ""
    $imageFilePath = "$workingDirectory\image$i.png"
    $pdfFilePath = "$workingDirectory\number$i.pdf"
...
#------------------create 1 pdf out of many pdf's----------------------------
$pdfFile = Get-Item "$workingDirectory\*.pdf"
$pdfFileName=$pdfFileName+".pdf"
Invoke-MergePdf -PdfSharpPath "C:\ProgramData\coolOrange\powerJobs\Modules\Pdf\PdfSharp-gdi.dll" -Files $pdfFile -DestinationFile "$workingDirectory\$pdfFileName"
